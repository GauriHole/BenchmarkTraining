...
var cartArr = [];
var cartItem = document.querySelector(".cart-count");
function updateCartUI() {
    ...
    if (cartArr.length === 0) {
        ...
        cartArr.forEach(function (item, index) {
            ...
            cartPrice.textContent = "$".concat(item.r);
            cartPrice.setAttribute("class", "cart-price");
            var cartTrash = document.createElement("i");
            cartTrash.setAttribute("class", "fa-solid fa-trash");
            cartTrash.style.cursor = "pointer";
            cartTrash.addEventListener("click", function () { return removeFromCart(index); });
            ...
            switch (_a.label) {
                ...
                case 2:
                    ...
                    data.forEach(function (ele) {
                        ...
                        var createTitleBox = document.createTextNode("".concat(ele.title.slice(0, 35), "..."));
                        var prodMainDiv = document.createElement("div");
                        var createPrice = document.createElement("p");
                        var createPriceBox = document.createTextNode(" Price : $".concat(ele.price));
                        ...
                        btnElement.addEventListener("click", function () { return addToCart(ele.image, ele.title, ele.price); });
                    });
                    return [2 /*return*/];
            }
        });
